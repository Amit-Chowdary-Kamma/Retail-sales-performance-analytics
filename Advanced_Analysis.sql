...
ORDER BY CONVERT(date, DATEADD(month, DATEDIFF(month, 0, order_date), 0));
...
SELECT 
  ...
  sub_category,
  total_sales,
  CONCAT(CAST((total_sales / SUM(total_sales) OVER()) * 100 AS varchar(50)), '%') AS perc_of_overall_sales
FROM category_sales
